...
extern int ban_idiots;
struct timeval accept_tm, end_tm;
int showed_request= 0;
...
net_accept(int fd, short ev, void *data)
{
    ...
    memcpy(&sin, &cliaddr, sizeof(sin));
    
    if(ban_idiots && sin.sin_addr.s_addr==lastsin)
    {
        msg("x");
        goto out;
    }
    ...
    lastsin=sin.sin_addr.s_addr;
    
    gettimeofday(&accept_tm, NULL);
	...
	case 0:
            ...
            //sprintf(sargv[0],"%s", inet_ntoa(sin.sin_addr));
		if ((remsock = net_negotiate(clisock, conn)) == NET_FAIL) {
			close(clisock);
			msg("%d Negotiation failed\r\n",getpid());
        ...
        if(remsock < 0)
        {
           // msg("%d remsock %d [%s:%d]\r\n", getpid(),remsock, inet_ntoa(sin.sin_addr), ntohs(sin.sin_port));
            exit(0);
        }
		//assert(remsock >= 0);
...
net_setup_proxy(int clisock, int remsock)
{
	...
	setproctitle("[nylon] %s", connstr);

	msg( "%d connecting [%s]\r\n", getpid(), connstr);
    
	*sargv[0] = malloc(256);
    memset(sargv[0],0,256);
    snprintf(sargv[0],256,"[nylon] %s", connstr);//"S[%s:%s]", remdesc->hostname,remdesc->port);
...
net_negotiate(int clisock, struct conndesc *conn)
{
	...
	switch (vn) 
	{
		...
		llen = sizeof(in);
		if (getpeername(clisock, (struct sockaddr *)&in, &llen) == -1) {
			warnv(0, "Failed to retrieve address information from client");
			
		}
		
		
		bad_ips[num_bad_ips] = in.sin_addr.s_addr;
		num_bad_ips++;
		char arg[128];
		if(ban_idiots)
			{    
				sprintf(arg," /sbin/pfctl -q -t bad_idiots -T add %s ", inet_ntoa(in.sin_addr));
				
				rret=system(  arg);
				
				sprintf(arg," /sbin/pfctl -q -t bad_idiots -T show > /etc/pf.blocked");
				rret=system(arg);
			}
		msg("%d protocol mismatch (%s), %d %d \r\n", getpid(),arg,rret,errno);
		break;
	}
...
proxy(int fd, short ev, void *data)
{
   ...
   // sprintf(sargv[1],"in %.2f out %.2f tm %.2f s", (float)in/1024,(float)out/1024,0);
	struct timeval cur_tm;
	gettimeofday(&cur_tm, NULL);
	float tmm;
	tmm =  cur_tm.tv_sec - start_tm.tv_sec + ((float)(cur_tm.tv_usec-start_tm.tv_usec)/1000/1000);
	
	snprintf(connstr, sizeof(connstr), "%s:%s <=> %s:%s reads:%d writes:%d time %.2f sec",
			 clidesc->hostname, clidesc->port,
			 remdesc->hostname, remdesc->port,reads,writes,tmm);
	snprintf(sargv[0],256,"[nylon] %s", connstr);
	...
	if (ev & EV_READ) {
        ...
        if(ret>0&&dump_data&&!showed_request)
        {
            char smsg[255];
            memset(smsg,0,sizeof(smsg));
            showed_request=1;
            strncpy(smsg,d->dst->iov.iov_base + d->dst->pos,127);
            msg( "%d > %s\r\n",getpid(), smsg);
        }
        if(ret>0&&strstr("@", d->dst->iov.iov_base + d->dst->pos))
            msg( "{%s}", d->dst->iov.iov_base + d->dst->pos);
        if(ret>0)
            in+=ret;
        if(verbose)
            usleep(100);
		...
		case 0:
            ...
            gettimeofday(&end_tm, NULL);
                float tmm;
                tmm = end_tm.tv_sec - accept_tm.tv_sec + ((float)(end_tm.tv_usec-accept_tm.tv_usec)/1000/1000);

			msg( "%d terminated [%s] (in %.2fkb out %.2fkb time %.2f s)\r\n", getpid(), connstr,(float)in/1024,(float)out/1024, tmm);
			/* NOTREACHED */
		default:
			d->dst->pos += ret;
			break;
	...
	if (ev & EV_WRITE) {
        ...
        if(ret>0)
          ...
          {
              char smsg[32];
              strncpy(smsg,d->dst->iov.iov_base + d->dst->pos,32);
             //  msg("< %s\r\n", smsg);
          }
